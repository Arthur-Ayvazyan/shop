...
const transport = nodemailer.createTransport(sendgridTransport({
    auth: {
        api_key: process.env.MAILER_TRANSPORT_KEY
    }
}));
...
exports.postSignup = (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
        ...
        errors.array().forEach(elem => { validationErrors[elem.param] = elem.msg });
        return res.status(422)
            .render('auth/signup', {
                path: '/signup',
                pageTitle: 'Signup',
    ...
    bcrypt
        ...
        .then(result => {
            ...
            return transport.sendMail({
                to: email,
                from: 'arthur.ayvaz@gmail.com',
                subject: 'success',
                html: '<h1>Congrats! you have an account</h1>'
            });
        })
        .catch(err => {
            handleError(err, 500, next);
